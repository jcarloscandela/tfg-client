...
export class NodePage implements OnInit {
  ...
  async getPayloads(dev_id: string): Promise<void> {
    this.nodesService
      .getNodePayloads(dev_id)
      .subscribe((payloads) => (this.payloads = payloads));
  }
  ...
  createChart() {
    this.bars = new Chart(this.barChart.nativeElement, {
      ...
      data: {
        //labels: ['1', '2', '3', '4', '5'],
        labels: this.payloads.map(function (x) {
          console.log(x);
          return (
            new Date(x.payload_date).getHours() +
            ":" +
            new Date(x.payload_date).getMinutes()
          );
        }),
        datasets: [
          {
